...
The context is UI development in web. These UIs are typically included in clients that run in browsers, on mobile devices or on desktops computers. They often exchange data with servers in the JSON data format \citep{jsonformat} through HTTP. The client is referred to as \textit{front-end}, the server as \textit{back-end}. End users interact with those UIs to carry out tasks.
